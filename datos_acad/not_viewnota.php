			...
			if($bUltmat)
			{
				...
				{	
				 	if($vCod_cur != $aNotas['cod_cur'])
					{
						if($vContn > 6)
						{
							$aNotah[$vCod_cur]['new_row'] = TRUE;
						}
						if($vContn > 11)
						{
							$aNotah[$vCod_cur]['new_row2'] = TRUE;
						}
					...
					if(($aNotas['mod_not'] == '09' or $aNotas['mod_not'] == '17' or $aNotas['mod_not'] == '13' or $aNotas['mod_not'] == '12'  or $aNotas['mod_not'] == '05' or $aNotas['mod_not'] == '19') and $aNotas['not_cur'] > 10)
				...
				$vQuery .= "and (cod_esp = '00' or cod_esp = '{$sEstudia['cod_esp']}') and con_cur = '1') union ";
				$vQuery .= "(Select distinct cu.cod_cur, cu.nom_cur, cu.nom_ofi, cu.cod_esp, cu.crd_cur, cu.tip_cur, ";
				$vQuery .= "cu.niv_est, cu.sem_anu from $tNota no left join unapnet.curso cu on ";
...
/*					if(strlen($sPlan['nom_cur']) <= 65)
					...
					$vNom_cur2 = "";
					$i = 0;
					if(strlen($aCurso['nom_ofi'].$aNotac[$aCurso['cod_cur']]) >= 47)					
					{						
						$vExede = TRUE;
						if(strlen($aCurso['nom_ofi']) >= 47)
						{
							for($i = 47; $i > 0; $i--)
							{
...
//							$vNom_cur .= strlen($vNom_cur);
					...
					$sPDF[$vContador]['cod_cur'] = $aCurso['cod_cur'];
					...
					if($aNotah[$aCurso['cod_cur']]['new_row'])
					{
						$sPDF[$vContador]['new_row'] = TRUE;
						...
						$sPDF[$vContador]['not_cur7'] = $aNotah[$aCurso['cod_cur'].'7']['not_cur'];
						$sPDF[$vContador]['ano_per7'] = $aNotah[$aCurso['cod_cur'].'7']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'7']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'7']['per_aca']]['abr_per']):'');
						$sPDF[$vContador]['not_cur8'] = $aNotah[$aCurso['cod_cur'].'8']['not_cur'];
						$sPDF[$vContador]['ano_per8'] = $aNotah[$aCurso['cod_cur'].'8']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'8']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'8']['per_aca']]['abr_per']):'');
						$sPDF[$vContador]['not_cur9'] = $aNotah[$aCurso['cod_cur'].'9']['not_cur'];
						$sPDF[$vContador]['ano_per9'] = $aNotah[$aCurso['cod_cur'].'9']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'9']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'9']['per_aca']]['abr_per']):'');
						$sPDF[$vContador]['not_cur10'] = $aNotah[$aCurso['cod_cur'].'10']['not_cur'];
						$sPDF[$vContador]['ano_per10'] = $aNotah[$aCurso['cod_cur'].'10']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'10']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'10']['per_aca']]['abr_per']):'');
						$vContador++;
					}
					if($aNotah[$aCurso['cod_cur']]['new_row2'])
					{
						$sPDF[$vContador]['new_row2'] = TRUE;
						$sPDF[$vContador]['not_cur11'] = $aNotah[$aCurso['cod_cur'].'11']['not_cur'];
						$sPDF[$vContador]['ano_per11'] = $aNotah[$aCurso['cod_cur'].'11']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'11']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'11']['per_aca']]['abr_per']):'');
						$sPDF[$vContador]['not_cur12'] = $aNotah[$aCurso['cod_cur'].'12']['not_cur'];
						$sPDF[$vContador]['ano_per12'] = $aNotah[$aCurso['cod_cur'].'12']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'12']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'12']['per_aca']]['abr_per']):'');
						$sPDF[$vContador]['not_cur13'] = $aNotah[$aCurso['cod_cur'].'13']['not_cur'];
						$sPDF[$vContador]['ano_per13'] = $aNotah[$aCurso['cod_cur'].'13']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'13']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'13']['per_aca']]['abr_per']):'');
						$sPDF[$vContador]['not_cur14'] = $aNotah[$aCurso['cod_cur'].'14']['not_cur'];
						$sPDF[$vContador]['ano_per14'] = $aNotah[$aCurso['cod_cur'].'14']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'14']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'14']['per_aca']]['abr_per']):'');
						$sPDF[$vContador]['not_cur15'] = $aNotah[$aCurso['cod_cur'].'15']['not_cur'];
						$sPDF[$vContador]['ano_per15'] = $aNotah[$aCurso['cod_cur'].'15']['ano_aca'].(($aNotah[$aCurso['cod_cur'].'15']['per_aca']!='00')?('-'.$sPeriodo[$aNotah[$aCurso['cod_cur'].'15']['per_aca']]['abr_per']):'');
						$vContador++;
					}
				}
				$aNotah = "";
...
/*					$sFechaA = "";
                ...
                <tr <? if($vCont % 2 == 0) echo "class=\"celpar\" id=\"p\""; else echo "class=\"celinpar\" id=\"i\"";?> onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
                  ...
                  <td class="wordcen"><? if($sUsercoo['cod_usu'] == '0054')	{	?><a href="" onclick="del_nota('<?=$aNotas['pln_est']?>', '<?=$aNotas['cod_cur']?>', '<?=$aNotas['mod_not']?>', '<?=$aAnoper['ano_aca']?>', '<?=$aAnoper['per_aca']?>'); return false;" class="enlaceb"><img src="../images/drop.png" alt="Eliminar" width="16" height="16" /></a><?	}	?></td>
                </tr>
                <? $vCont++; 	} ?>
            </table></td>
...
<? } ?>		 
		...
		<? if($bUltmat)	{	?>
			...
			<a href="prnhistoacadconren.php" title="Imprimir Historial Academico" class="linkboton" target="frPdf"><img src="../images/bphacadcon.png" width="100" height="24"></a>	  
		<?	}	?>
