<?php
	...
	if(fsafetyselcar2())
	{
		class PDF extends FPDF
		{
			...
			function Subcuadro()
			{
				global $sUsercoo, $sCarrera, $sFacultad, $sEstudia, $sSemestre;
				$this->SetFont('arialn','B',10);
				//$this->Cell(10);
				$this->Cell(140,4,"FACULTAD: {$sFacultad[$sUsercoo['cod_fac']]}", 0);
				$this->SetFont('arial','B',13);
				$this->Cell(140,4,"QUINTO SUPERIOR [{$sUsercoo['ano_aca']}]", 0, 0, 0,'R');
				$this->SetFont('arialn','B',10);
				$this->Ln();
				...
				//$this->Cell(10);
				$this->Cell(140,4, "SEMESTRE: {$sSemestre[$sUsercoo['sem_anu']]} (ESTUDIANTES QUE APROBARON ENTRE {$sUsercoo['crd_ini']} y {$sUsercoo['crd_fin']} CRÉDITOS)", 0);				
//				$this->Ln();
				
				//$this->Cell(10);
				$this->Cell(140,4, "QUINTO SUPERIOR EMITIDO SEGÚN: RESOLUCIÓN RECTORAL N° 1682-2006-R-UNA", 0, 0, 0,'R');				
				$this->Ln();		
				
				//$this->Cell(10);
				$this->Cell(140,4, "ALCANCE: Art. 07 Para el cálculo del quinto superior solo se considera a la quinta parte de los estudiantes que aprobaron más de 24 créditos consignados en el Cuadro de Méritos", 0);				
				$this->Ln();				
