<?php
	...
	{		
		...
		$vPln_esp = $_GET['rPln_esp'];
		$sUsercoo['pln_esp'] = $vPln_esp;
		if(empty($vPln_esp))
			header("Location:crd_selecesp.php");
...
<body onLoad="start();" onUnload="window.returnValue = document.fData.rReturn.value">
	...
	<div class="headerow">
		...
		<div class="headermsn"><span class="wordizqb">[<?=$sCarrera[$sUsercoo['cod_car']]?>
	    - Cuadro de M&eacute;ritos - Esp./Menci&oacute;n : <?=ucwords(strtolower($sEspecial[$vPln_est.$vCod_esp]['esp_nom']))?>]</span></div>
...
/*	$vQuery = "select no.num_mat, no.pln_est, sum(cu.crd_cur) as all_crd, ";
	...
	$vQuery .= "where no.not_cur > 10 and no.ano_aca <= '{$sUsercoo['ano_aca']}' and no.pln_est = '{$aPlan['pln_est']}' and ";
	$vQuery .= "no.num_mat in (select distinct num_mat from $tEstumat) group by num_mat, pln_est ";
	$vQuery .= "having all_crd >= '$vCrd_ini' and all_crd <= '$vCrd_fin'";*/
	
	$vQuery = "select em.num_mat, concat(es.paterno, ' ', es.materno, ', ', es.nombres) as nombre, em.pln_est ";
	$vQuery .= "from $tEstumat em left join unapnet.estudiante es on em.cod_car = es.cod_car and em.num_mat = es.num_mat ";
	$vQuery .= "where em.pln_est = '$vPln_est' and em.cod_esp = '$vCod_esp' and per_aca = '{$sUsercoo['per_aca']}'";
...
//	$cPlan->close();
	...
	foreach($sEstucdo as $vNum_mat => $aEstucdo)
	{
		$vPer_aca = 2;
		// -----------------------------------MOd mat----------------
		$vQuery = "select per_aca, mod_mat, cod_esp, tot_crd from $tEstumat where num_mat = '$vNum_mat' and ";
		$vQuery .= "(per_aca = '01' or per_aca = '02') order by per_aca"	;
		$cEstumat = fQuery($vQuery);
		while($aEstumat = $cEstumat->fetch_array())
		{
			if($aEstumat['per_aca'] == '01')
				$sEstucdo[$vNum_mat]['mod_mat1'] = $aEstumat['mod_mat'];
			if($aEstumat['per_aca'] == '02')
				$sEstucdo[$vNum_mat]['mod_mat2'] = $aEstumat['mod_mat'];
			$vPer_aca -= 1;
		...
		$vQuery .= "cm.cod_cur = cu.cod_cur where cm.num_mat = '$vNum_mat' and (per_aca = '01' or per_aca = '02') ";
		$vQuery .= "group by per_aca";
		$cCurmat = fQuery($vQuery);
		while($aCurmat = $cCurmat->fetch_array())
		{
			if($aCurmat['per_aca'] == '01')
				$sEstucdo[$vNum_mat]['crd_mat1'] = round($aCurmat['tot_crd'], 1);
			if($aCurmat['per_aca'] == '02')
				$sEstucdo[$vNum_mat]['crd_mat2'] = round($aCurmat['tot_crd'], 1);
		}
		$cCurmat->close();		
		...
		$vQuery .= "(no.per_aca = '01' or per_aca = '02') and (no.mod_not = '01' or no.mod_not = '07' or no.mod_not = '10' or ";
		$vQuery .= "no.mod_not = '11' or no.mod_not = '14' or no.mod_not = '15' or no.mod_not = '16' or no.mod_not = '18') ";
		$vQuery .= "group by per_aca";
		$cNota = fQuery($vQuery);
		while($aNota = $cNota->fetch_array())
		{
			...
			if($aNota['per_aca'] == '02')
			{
				$sEstucdo[$vNum_mat]['crd_apb2'] = round($aNota['crd_apb'], 1);
				$sEstucdo[$vNum_mat]['crd_des2'] = round($aNota['crd_des'], 1);
				$sEstucdo[$vNum_mat]['crd_con2'] = $aNota['tot_crd'];
				$sEstucdo[$vNum_mat]['crd_not2'] = $aNota['crd_not'];
				if($aNota['tot_crd'] > 0)
					$sEstucdo[$vNum_mat]['prd_sem2'] = (float)round($aNota['crd_not']/$aNota['tot_crd'], 2);
			}
		}
		$cNota->close();
		...
		// ----------------------------Crd mat > 24-----------------------
		if($sEstucdo[$vNum_mat]['crd_mat'] > 24)
			$sEstukey[$vNum_mat] .= '0';
		else
			$sEstukey[$vNum_mat] .= '1';
		
		// ----------------------------Crd Aprobados-----------------------
		if($sEstucdo[$vNum_mat]['crd_apb'] > 30)
			$sEstukey[$vNum_mat] .= '0';
		elseif($sEstucdo[$vNum_mat]['crd_apb'] > 24 && $sEstucdo[$vNum_mat]['crd_apb'] <= 30)
			$sEstukey[$vNum_mat] .= '1';
		elseif($sEstucdo[$vNum_mat]['crd_apb'] > 20 && $sEstucdo[$vNum_mat]['crd_apb'] <= 24)
			$sEstukey[$vNum_mat] .= '2';
		elseif($sEstucdo[$vNum_mat]['crd_apb'] > 10 && $sEstucdo[$vNum_mat]['crd_apb'] <= 20)
			$sEstukey[$vNum_mat] .= '3';
		elseif($sEstucdo[$vNum_mat]['crd_apb'] <= 10)
			$sEstukey[$vNum_mat] .= '4';
  ...
  ?>
	...
	<table border="0" cellpadding="0" cellspacing="0" id="ventana">
        ...
        <tr>
		 ...
		 <table width="868" border="1" cellpadding="1" cellspacing="0" bordercolor="#BDD37B" rules="cols, rows" class="tabled">
            <tr>
              ...
              <th colspan="5">SEMESTRE I </th>
              <th colspan="5">SEMESTRE II</th>
              <th>&nbsp;</th>
            </tr>
            <tr>
              <th width="35">PPS</th>
	  ...
	  <a href="prncuadrofesp.php" title="Imprimir" class="linkboton" target="frPdf"><img src="../images/bprint.png" width="100" height="24" border="0" /></a>	    
