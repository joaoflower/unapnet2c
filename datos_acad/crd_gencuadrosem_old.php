...
//	$vQuery = "Select distinct pln_est from $tEstumat where (per_aca = '01' or per_aca = '02') order by pln_est";
	...
	foreach($sEstucdo as $vNum_mat => $aEstucdo)
	{
		...
		$vQuery .= "no.mod_not = '11' or no.mod_not = '14' or no.mod_not = '15' or no.mod_not = '16' or no.mod_not = '18' or ";
		$vQuery .= "no.mod_not = '19' ) group by per_aca";
		$cNota = fQuery($vQuery);
...
//------------			*/
		...
		if($sEstucdo[$vNum_mat]['crd_apb'] >= $sUsercoo['crd_sem'])
			$sEstukey[$vNum_mat] .= '0';
		else
			$sEstukey[$vNum_mat] .= '1';
