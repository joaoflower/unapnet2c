<?php
	...
	if(fsafetyselcar2())
	{
		class PDF extends FPDF
		{
			...
			function Subhnota()
			{
				global $sUsercoo, $sCarrera, $sFacultad, $sPeriodo, $sModmat, $sSemestre, $sGrupo, $sEstudia, $sEspecial, $sModnot, $sNivel;
				$this->SetFont('arial','B',8);
				$this->Cell(5);
				$this->Cell(130,4,"FACULTAD: {$sFacultad[$sUsercoo['cod_fac']]}", 0);
				$this->Cell(50,4,"ESTUDIANTES [{$sUsercoo['ano_aca']}-{$sPeriodo[$sUsercoo['per_aca']]['per_des']}]", 0, 0, 0,'R');
				$this->Ln();
				
				$this->Cell(5);
				$this->Cell(180,4, "ESCUELA PROFESIONAL: {$sCarrera[$sUsercoo['cod_car']]}, {$sEspecial[$sEstudia['pln_est'].$sEstudia['cod_esp']]['esp_nom']}", 0);
				$this->Ln();
				
				$this->Cell(5);
				$this->Cell(180,4, "CURSO: {$sEstudia['nom_cur']}", 0);
				$this->Ln();
				
				//$vNombre = strtoupper("{$sUser['paterno']} {$sUser['materno']}, {$sUser['nombres']}");
				$this->Cell(5);
				$this->Cell(180,4, "DOCENTE: {$sEstudia['nom_doc']}", 0);
				$this->Ln();
			
				$this->Cell(5);
				$this->Cell(180,4, "NIVEL: {$sNivel[$sEstudia['niv_est']]}   -   SEMESTRE: {$sSemestre[$sEstudia['sem_anu']]}   -   CREDITOS: {$sEstudia['crd_cur']}   -   GRUPO: {$sGrupo[$sEstudia['sec_gru']]}   -   MODALIDAD: {$sModnot[$sEstudia['mod_not']]}", 0);
				$this->Ln();
