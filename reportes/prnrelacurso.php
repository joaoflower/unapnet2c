<?php
	...
	if(fsafetyselcar2())
	{
		class PDF extends FPDF
		{
			...
			function Subhnota()
			{
				global $sUsercoo, $sCarrera, $sFacultad, $sPeriodo, $sModmat, $sSemestre, $sGrupo, $sEstudia, $sEspecial, 
				$sModnot, $sNivel;
				...
				$this->Cell(5);
				$this->Cell(180,4, "NIVEL: {$sNivel[$sEstudia['niv_est']]}   -   SEMESTRE: {$sSemestre[$sEstudia['sem_anu']]}   -   CREDITOS: {$sEstudia['crd_cur']}   -   GRUPO: {$sGrupo[$sEstudia['sec_gru']]}   -   MODALIDAD: {$sModnot[$sEstudia['mod_act']]}", 0);
				$this->Ln();
