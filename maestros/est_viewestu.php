...
<table border="1" cellpadding="1" cellspacing="0" bordercolor="#BDD37B" rules="cols, rows" class="tabled">
	...
	if($bDatos == TRUE)
	{
		...
		while($aEstudia = $cEstudia->fetch_array())
		{
			...
			$sUsercoo['cod_car2'] = $aEstudia['cod_car'];
  	?>
  <tr <? if($vCont % 2 == 0) echo "class=\"celpar\" id=\"p\""; else echo "class=\"celinpar\" id=\"i\"";?> onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
    ...
    <td width="60"><a href="" onClick="viewdataest('<?=$aEstudia['num_mat']?>', '<?=$aEstudia['cod_car']?>'); return false;" class="enlace1">
      <?=$aEstudia['num_mat']?>
    </a></td>
    ...
    <td width="16" class="wordizq"><a href="" onClick="viewdataest('<?=$aEstudia['num_mat']?>', '<?=$aEstudia['cod_car']?>'); return false;" class="enlaceb"><img src="../images/browse.png" alt="Mostrar informaci&oacute;n" width="16" height="16" /></a></td>
  </tr>
  	<? 
		$vCont++; 	
		} 
	...
	?>
<input name="rCan_est" type="hidden" value="<?=$sUsercoo['can_est']?>">
<input name="rNum_mat2" type="hidden" value="<?=$sUsercoo['num_mat']?>">
<input name="rCod_car2" type="hidden" value="<?=$sUsercoo['cod_car2']?>">
</table>
