...
<?php
	...
	{		
		if($sUsercoo['safetymatri'])
		{
			...
			$vRes_cam = $_GET['rRes_cam'];
			
			$vFecha = getdate(time());
			$vFechan = "{$vFecha['mday']} de {$sMes[$vFecha['mon']]} del {$vFecha['year']} - Hora: {$vFecha['hours']}:{$vFecha['minutes']}:{$vFecha['seconds']} hrs.";
			
			$vObs_est = $sEstudia['obs_est'];
			$vObs_est .= "<<<  CAMBIO DE NOMBRE  >>> \n";
			$vObs_est .= " - Anterior: [{$sEstudia['paterno']} {$sEstudia['materno']}, {$sEstudia['nombres']}]\n";
			$vObs_est .= " - Nuevo: [$vPaterno $vMaterno, $vNombres]\n";
			$vObs_est .= " - Documento: [$vRes_cam]\n";
			$vObs_est .= " - IP: [{$sUsercoo['ip']}]. \n";
			$vObs_est .= " - Fecha: [$vFechan]. \n";
			
			$vQuery = "update unapnet.estudiante set paterno = '$vPaterno', materno = '$vMaterno', ";
			$vQuery .= "nombres = '$vNombres', obs_est = '$vObs_est' ";
			$vQuery .= "where num_mat = '{$sEstudia['num_mat']}' and cod_car = '{$sEstudia['cod_car']}'";
			
			$bUpdate = fInupde($vQuery);
			if($bUpdate)
			{
				$vQuery = "update unap.estudiante set paterno = '$vPaterno', materno = '$vMaterno', ";
				$vQuery .= "nombres = '$vNombres' ";
				$vQuery .= "where num_mat = '{$sEstudia['num_mat']}' and cod_car = '{$sEstudia['cod_car']}'";				
				$bUpdate = fInupde($vQuery);
