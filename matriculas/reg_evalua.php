...
<?php
	...
	if(fsafetyselcar())
	{
		...
		$vQuery .= "from unapnet.apeestu where num_mat = '$vNum_mat' and cod_car = '$vCod_car' and ";
		...
		$vQuery = "Select num_mat, paterno, materno, nombres, passwd, ult_mat, ult_per ";
		...
		{			
			...
			$sEstudia['ult_mat'] = $aEstudia['ult_mat'];
			$sEstudia['ult_per'] = $aEstudia['ult_per'];
	...
	/*					$vQuery = "select num_reb from $tPago where ano_aca = '{$sUsercoo['ano_aca']}' and ";
					...
					if($sEstudia['ult_mat'] != '2009') //and $sEstudia['ult_per'] != '01')
					{
						$vCan_pag = 0;
						$vQuery = "Select can_pag from unapnet.tarifanmis ";
						$vQuery .= "where ult_mat = '{$sEstudia['ult_mat']}' and ult_per = '{$sEstudia['ult_per']}' ";
						
						$cTarifanmis = fQuery($vQuery);
						if($aTarifanmis = $cTarifanmis->fetch_array())
						{
							$vCan_pag = $aTarifanmis['can_pag'];
						}
						$cTarifanmis->close();
					...
					?>
					<tr class="celinpar">
					  <td class="wordcen" scope="col">24</td>
					  <td class="wordizq" scope="col">&nbsp;<?=$sTarifapago['24']['des_pag']?></td>
					  <td class="wordcen" scope="col"><?=$vCan_pag?></td>
					  <td class="wordder" scope="col"><?=$sTarifapago['24']['mon_pag']?>&nbsp;</td>
					  <td class="wordderb" scope="col"><?=($sTarifapago['24']['mon_pag']*$vCan_pag)?>&nbsp;</td>
				  </tr>
					<?
						}
					}
				...
				<?
					...
					//$vPag_caja
				?>
				<span class="wordi">S/.(VERIFIQUE EL RECIBO)</span><br />
				<?
				if($bPago) {
				?>
