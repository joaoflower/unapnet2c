...
<?php
	...
	if(fsafetyselcar())
	{
		...
		$vQuery = "Select num_mat, cod_car, paterno, materno, nombres from unapnet.estudiante where cod_car = '{$sUsercoo['cod_car']}' and con_est = '1' ";
...
<table border="1" cellpadding="1" cellspacing="0" bordercolor="#BDD37B" rules="cols, rows" class="tabled">
  ...
  <tr <? if($vCont % 2 == 0) echo "class=\"celpar\" id=\"p\""; else echo "class=\"celinpar\" id=\"i\"";?> onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
    ...
    <td width="60"><a href="" onClick="evaluamreg('<?=$aEstudia['num_mat']?>', '<?=$aEstudia['cod_car']?>'); return false;" class="enlace1">
      <?=$aEstudia['num_mat']?>
    </a></td>
    ...
    <td width="16" class="wordizq"><a href="" onClick="evaluamreg('<?=$aEstudia['num_mat']?>', '<?=$aEstudia['cod_car']?>'); return false;" class="enlaceb"><img src="../images/browse.png" alt="Mostrar informaci&oacute;n" width="16" height="16" /></a></td>
  </tr>
  	<? 
		$vCont++; 	
		} 
