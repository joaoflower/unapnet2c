<?php
	...
	if(fsafetyselcar())
	{
		...
		$vQuery .= "$tEstumat.cod_car = estudiante.cod_car where estudiante.cod_car = '{$sUsercoo['cod_car']}' ";
		
		if((!empty($vNum_mat) or !empty($vPaterno) or !empty($vMaterno) or !empty($vNombres)))
		{
			if(!empty($vNum_mat))
				$vQuery .= "and estudiante.num_mat = '$vNum_mat' ";
			if(!empty($vPaterno))
				$vQuery .= "and estudiante.paterno = '$vPaterno' ";
			if(!empty($vMaterno))
				$vQuery .= "and estudiante.materno = '$vMaterno' ";
			if(!empty($vNombres))
				$vQuery .= "and estudiante.nombres = '$vNombres' ";
...
<body >
	...
	<div class="wordcen" id="body1">
		<form action="edt_getestu.php" method="post" enctype="multipart/form-data" name="fData" id="fData"> 
	  <table border="0" cellpadding="0" cellspacing="0" id="ventana">
        <tr>
          <th><img name="ventana_r1_c1" src="../images/ventana_r1_c1.jpg" width="12" height="29" border="0" alt=""></th>
          <th align="center" background="../images/ventana_r1_c2.jpg" >Estudiante Matriculados </th>
        ...
        <tr>
          ...
          <td background="../images/ventana_r2_c2.jpg" class="wordcen"><table border="1" cellpadding="1" cellspacing="0" bordercolor="#BDD37B" rules="cols, rows" class="tabled">
			...
			<tr <? if($vCont % 2 == 0) echo "class=\"celpar\""; else echo "class=\"celinpar\"";?>>
              <td><a href="edt_viewmatri.php?rNum_mat=<?=$vNum_mat2?>" class="enlace1"><?=$vNum_mat2?></a></td>
